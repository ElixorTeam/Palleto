...
const handleBarcodeEnter = async (
	...
	if (event.key === "Enter") {
		if (barcode)
			await dotNetObjectReference.invokeMethodAsync(functionName, barcode);
		barcode = "";
		return;
	}
 ...
 */
window.subscribeBarcodeEnterEvent = (
	dotNetObjectReference: DotNetObjectType,
	functionName: string,
): void => {
	inputEventHandler = (event) => {
		handleBarcodeEnter(event, dotNetObjectReference, functionName).catch(
			(error: unknown) => {
