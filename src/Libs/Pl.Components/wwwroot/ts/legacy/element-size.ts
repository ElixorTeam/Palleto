...
const resizeElement = (element: Element): void => {
	const button = element.querySelector(".width-sub-element") as
		| HTMLElement
		| undefined;
	const dropdown = element.querySelector(".width-ref-element") as
		| HTMLElement
		| undefined;
	if (dropdown && button)
		dropdown.style.width = `${button.offsetWidth.toString()}px`;
};
 ...
 * @return {void} This function does not return anything.
 */
window.subscribeElementResize = (
	element: ElementWithHandler | undefined,
): void => {
	if (!element) return;
	const resizeHandler = () => {
...
window.addDotNetEventListener = (
	...
	const handler: EventListener = () => {
		dotNetObjectReferencer
			.invokeMethodAsync(functionName, ...arguments_)
			.catch((error: unknown) => {
				console.log(error);
			});
	};
	...
	eventHandlers.push({
		dotNetObjectId: dotNetObjectReferencer._id,
		functionName,
		eventName,
		handler,
	});
...
window.removeDotNetEventListener = (
	...
	const index = eventHandlers.findIndex(
		(h) =>
			h.dotNetObjectId === dotNetObjectReferencer._id &&
